...
class TemplateSampler(torch.nn.Module):
    def __init__(self, grid_xz_bev, ppm, num_rotations, optimize=True):
        ...
        # angles = torch.zeros(1, device=grid_xz_bev.device)#这里修改为无旋转
...
def conv2d_fft_batchwise(signal, kernel, padding="same", padding_mode="constant"):
    ...
    # print(signal_fr.shape, kernel_fr.shape)
...
def sample_xyr(volume, xy_grid, angle_grid, nearest_for_inf=True):
    ...
    grid_norm = grid * 2 - 1 #把gt归一化

    valid = torch.all((grid_norm >= -1) & (grid_norm <= 1), -1)
    value = grid_sample(volume_padded, grid_norm, align_corners=True, mode="bilinear")#用归一化后的grid在volum中采样，通过 grid_sample 可以将模型输出与volum的真实值进行比较，并计算出细致的损失。
    ...
    return value, valid
def sample_xyr_new(volume, xy_grid, nearest_for_inf=True):
    # (B, C, H, W, N) to (B, C, H, W, N+1)
    # volume换成4D的，只对xy进行采样
    # volume_padded = pad(volume, [0, 1, 0, 0, 0], mode="circular")
    volume= volume.permute(0, 3, 2, 1)
    size = xy_grid.new_tensor(volume.shape[-2:-1])
    xy_norm = xy_grid / (size - 1)  # align_corners=True #归一化的xy
    # angle_norm = (angle_grid / 360) % 1 #归一化的angle
    # grid = torch.concat([angle_norm.unsqueeze(-1), xy_norm], -1)
    grid = xy_norm # 只包含 x 和 y 的信息
    grid_norm = grid * 2 - 1 #把gt归一化
    grid_norm = grid_norm.squeeze(0)
    # print("grid_norm size:", grid_norm.size())# 如果 batch_size 是 1，去掉 batch 维度
    # grid_norm = grid_norm.squeeze(0)
    # print("grid_norm size:", grid_norm.size())
    valid = torch.all((grid_norm >= -1) & (grid_norm <= 1), -1)
    value = grid_sample(volume, grid_norm, align_corners=True, mode="bilinear")
...
def nll_loss_xyr(log_probs, xy, angle):
    ...
    return nll
def nll_loss_xy(log_probs, xy, angle, angle_loss_weight=0.1):
    log_prob, _ = sample_xyr_new(
        log_probs, xy[:, None, None, None]
    )
    nll = -log_prob.reshape(-1)  # remove C,H,W,N
    return nll
def nll_loss_xy_radar(log_probs, xy, angle, angle_loss_weight=0.1):
    log_prob, _ = sample_xyr_new(
        log_probs, xy[:, None, None, None]
    )
    nll = -log_prob.reshape(-1)  # remove C,H,W,N
...
def argmax_xyr(scores: torch.Tensor) -> torch.Tensor:
    ...
    return xyr
def argmax_xy(scores: torch.Tensor) -> torch.Tensor:
    # indices = scores.view(scores.size(0), -1).argmax(-1)
    indices = scores.reshape(scores.size(0), -1).argmax(-1)
    width = scores.size(-2)
    y = indices // width
    x = indices % width
