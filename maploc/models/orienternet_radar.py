...
class OrienterNet(BaseModel):
    ...
    def exhaustive_voting(self, f_bev, f_map, valid_bev, confidence_bev=None):
        if self.conf.normalize_features:
            f_bev = normalize(f_bev, dim=1)#【1，8，64，129】
        ...
        if confidence_bev is not None:
            f_bev = f_bev * confidence_bev.unsqueeze(1) #【1，8，64，129】
        f_bev = f_bev.masked_fill(~valid_bev.unsqueeze(1), 0.0)#[1,8,64,129]
        # f_bev = f_bev.masked_fill(torch.logical_not(valid_bev.unsqueeze(1)), 0.0)
        templates = self.template_sampler(f_bev)#[1,64,8,129,129]
    ...
    def _forward(self, data):
        ...
        scores = scores.moveaxis(1, -1)  # B,H,W,N 【1，256，256，64】
        ...
        # level = 0
        # f_image = self.image_encoder(data)["feature_maps"][level]
        # # print("f_image 维度:", f_image.shape)
        # # 调整f_image的高度，使其与f_bev的高度匹配
        # conv_adjust = nn.Conv2d(in_channels=f_image.shape[1], out_channels=f_image.shape[1], kernel_size=(1, 2))
        # conv_adjust = conv_adjust.to(f_image.device)
        # f_image1 = conv_adjust(f_image)
        # f_image1 = F.interpolate(f_image1, size=(64, 129), mode='bilinear', align_corners=False)
        # # 打印调整后的f_image的维度
        # # print("调整后的 f_image 维度:", f_image.shape)
        # # 跳过BEV投影，直接使用图像特征
        # # ff_bev = f_image
        # # validd_bev = torch.ones_like(ff_bev)  # 一个虚拟的掩码，因为BEV被跳过了

        # # # 打印f_bev的维度
        # # print("f_bev 维度:", ff_bev.shape)

        # # # 打印valid_bev的维度
        # # print("f_valid 维度:", validd_bev.shape)
        # # 去掉了投影的部分，radar图片本身是俯瞰图
        # # camera = data["camera"].scale(1 / self.image_encoder.scales[level])
        # # camera = camera.to(data["image"].device, non_blocking=True)
        ...
        # # BEV特征f_bev和有效区域的掩码valid_bev，雷达部分跳过，但是要得到一个mask
        # # Map to the BEV.
        # # with torch.autocast("cuda", enabled=False):
        # #     f_bev, valid_bev, _ = self.projection_bev(
        # #         f_polar.float(), None, camera.float()
        # #     )
        # # print("f_bev 维度:", f_bev.shape)
        # # print("f_valid 维度:", valid_bev.shape)
        # f_bev = f_image1
    ...
    def loss(self, pred, data):
        ...
        if self.conf.do_label_smoothing:
            ...
            # nll = nll_loss_xy(pred["log_probs"], xy_gt, yaw_gt)
