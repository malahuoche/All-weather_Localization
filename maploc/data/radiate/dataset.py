...
class RadiateDataModule(pl.LightningDataModule):
    ...
    def get_frame_data(self, date, drive, index):
        ...
        gps_path = (
            self.root / date / "gps_new" / Path(index).with_suffix(".txt")
        )
        _, R_world_gps, t_world_gps,yaw_angle = parse_gps_file(
        ...
        roll_pitch_yaw = np.array([-roll, -pitch, yaw_angle+90], np.float32)  # for some reason
