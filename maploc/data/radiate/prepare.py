...
# from .dataset import RadiateDataModule
import sys
...
from maploc.data.radiate.utils import parse_gps_file
from maploc.data.radiate.dataset import RadiateDataModule
...
def prepare_osm(
    ...
    all_latlon = []
    for gps_path in data_dir.glob("city_*/GPS_IMU_Twist/*.txt"):#修改为radiate的gps文件，对应目录下访问
    ...
    plotter.fig.write_html(data_dir / "split_radiate.html")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=Path, default=Path(RadiateDataModule.default_cfg["data_dir"])
    )
    ...
    tiles_path = args.data_dir / RadiateDataModule.default_cfg["tiles_filename"]
    if args.generate_tiles:
        ...
        osm_path = Path("/home/classlab2/root/OrienterNet/datasets/OSM/map15.osm")
