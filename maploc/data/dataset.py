...
class MapLocDataset(torchdata.Dataset):
    ...
    def __init__(
        ...
        self.sensor_type = cfg.sensor_type
        self.data = data
    ...
    def get_sensor_image_path(self,base_path, sensor_type):
        """
        Modifies and returns the correct image path based on the sensor type.
        :param base_path: The base image path.
        :param sensor_type: The type of sensor, such as 'radar', 'lidar', 'camera', etc.
        :return: The modified image path.
        """
        if sensor_type == 'radar_camera':
            return base_path.replace("/radar/cart", "/camera_new_r"), base_path.replace("cart", "cart_ro")
        elif sensor_type == 'lidar_camera':
            return base_path.replace("/radar/cart", "/camera_new_r"), base_path.replace("/radar/cart", "/lidar_cart")
        elif sensor_type == 'camera_only':
            return base_path.replace("/radar/cart", "/camera_new_r"), base_path
        elif sensor_type == 'radar_only':
            return base_path.replace("/radar/cart", "/radar/cart_ro"), base_path
        else:
            # 如果传感器类型未知，返回原始路径或抛出异常
            return base_path
    ...
    def get_view(self, idx, scene, seq, name, seed, bbox_tile):
        ...
        # print(image_path)
        camera_image_path,radar_image_path = self.get_sensor_image_path(image_path, self.sensor_type)
