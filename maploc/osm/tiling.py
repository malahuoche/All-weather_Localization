...
class MapIndex:
    ...
    def query(self, bbox: BoundaryBox) -> Tuple[List[OSMNode], List[OSMWay]]:
        ...
        for x in ["nodes", "lines", "areas"]:
            ...
            # print(f"{x} ids:", ids)
            elements = [getattr(self.data, x)[i] for i in ids]
            # print(f"{x} elements:", elements)
            ret.append(elements)
...
class TileManager:
    ...
    def from_bbox(
        ...
        bbox_osm = projection.unproject(bbox)#bbox_osm是经纬度坐标
        if path is not None and path.is_file():
            ...
            osm = OSMData.from_dict(get_osm(bbox_osm, path))#如果提供了osm文件路径，将使用该文件进行地图生成；否则，将从提供的GPS坐标信息中获取数据以生成地图。

        osm.add_xy_to_nodes(projection)#把投影后的坐标xy添加到osm中
        # 假设 osm 是一个 OSMData 对象
        # for node_id, node in osm.nodes.items():
        #     node_coordinates = node.xy  # 假设 OSMNode 类中有 xy 属性
        #     print(f"Node {node_id} coordinates: {node_coordinates}")
    ...
    def query(self, bbox: BoundaryBox) -> Canvas:
        ...
        for i in range(ij_min[0], ij_max[0] + 1):
            for j in range(ij_min[1], ij_max[1] + 1):
                ...
                # print("Slice Query:", slice_query)
                # print("Slice Tile:", slice_tile)
                # print("Tile Raster Summary:", tile.raster.min(), tile.raster.max())
