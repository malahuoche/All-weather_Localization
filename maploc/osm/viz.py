...
class Colormap:
    ...
    @classmethod
    def apply_lines(cls, rasters):

        # way_color = cls.colors_ways[rasters[1]] / 255.0
        way_color = np.array([255, 255,255])
            
        area_color = np.ones_like(cls.colors_areas[0])

        return np.where(rasters[1, ..., None] > 0, way_color, area_color)
